"""experiments/run_hierarchical_v7.py

v7: v6 hierarchical cascade + 3 组新加的、有文献依据的手工特征模块.

新增的可组合积木 (相对 v6):
    lbp_multi   - 多尺度 LBP (R=1, R=3). Ojala et al. 2002; Barata et al. 2014
    gabor       - 4 方向 x 3 频率 Gabor 滤波响应. Daugman 1985; Sadeghi et al. 2013
    subregion   - 4 象限颜色不对称. Stolz et al. 1994 (ABCD); Celebi et al. 2007

默认 Stage 2 特征改为:
    all+boundary+melnv+lbp_multi+gabor+subregion
让 SelectKBest 在更大的池子里挑出最强的 k 个 mel/nv 鉴别特征.

依赖: pip install xgboost
缓存: 复用 features_v2_*.csv (新组合首次会触发提取)

命令行示例:
    # 5 种子完整跑 (新特征第一次提取会慢, 约 2-3 分钟; 之后缓存命中秒级)
    python experiments/run_hierarchical_v7.py --data_dir data\Data_Proj2

    # 单种子快验
    python experiments/run_hierarchical_v7.py --data_dir data\Data_Proj2 --seeds 127
"""
...
def resolve_modules(feature_set):
    ...
    for part in parts:
        if part == "all":
            ...
            raise ValueError(
                f"Unknown block '{part}'. Allowed: 'all', "
                f"{list(ATOMIC_EXTRACTORS)}"
            )
    seen, deduped = set(), []
    for m in modules:
        if m not in seen:
...
# ----------------------------------------------------------------------------
# 主流程: cascade + flat 基线 (与 v6 等价, 仅默认 spec 不同)
# ----------------------------------------------------------------------------
def run_hierarchical_v7(data_dir, stage1_spec, stage2_spec, flat_spec,
    ...
    print(f">>> Stage 1: {stage1_spec['classifier']:<5s} on "
          f"{stage1_spec['feature_set']:<50s} k={stage1_spec['k_features']}")
    print(f">>> Stage 2: {stage2_spec['classifier']:<5s} on "
          f"{stage2_spec['feature_set']:<50s} k={stage2_spec['k_features']}")
    print(f">>> Flat   : {flat_spec['classifier']:<5s} on "
          f"{flat_spec['feature_set']:<50s} k={flat_spec['k_features']}")
    print(f">>> Mode: {mode}   vasc_threshold (hard only): {vasc_threshold}")
    print(f">>> Seeds: {seeds}\n")
    ...
    for seed in seeds:
        ...
        for tr, va in splits:
            ...
            proba_s1 = _fit_predict_proba(stage1_spec, X_s1.iloc[tr], y_tr_s1,
                                          X_s1.iloc[va], n_classes=2)
            p_vasc = proba_s1[:, 1]
            ...
            y_tr_s2 = (y_tr_str[mn_mask_tr] == "nv").astype(int)
            proba_s2 = _fit_predict_proba(stage2_spec, X_tr_s2, y_tr_s2,
                                          X_s2.iloc[va], n_classes=2)
            p_mel = proba_s2[:, 0]
            p_nv = proba_s2[:, 1]

            mn_mask_va = (y_va_str != "vasc")
            if mn_mask_va.any():
                s2_pred = np.where(p_mel[mn_mask_va] > p_nv[mn_mask_va], "mel", "nv")
            ...
            proba_flat = _fit_predict_proba(flat_spec, X_flat.iloc[tr], y_tr_flat,
                                            X_flat.iloc[va], n_classes=3)
    ...
    print(pd.DataFrame(m_c_bag["confusion_matrix"], index=LABELS, columns=LABELS).to_string())
    print("\n  Flat confusion matrix (rows=true, cols=pred):")
    print(pd.DataFrame(m_f_bag["confusion_matrix"], index=LABELS, columns=LABELS).to_string())
...
def plot_methods(result_df, output_png, metric="balanced_accuracy"):
    ...
    bars[best_idx].set_edgecolor("#1f1f1f")
    bars[best_idx].set_linewidth(1.8)
    ax.text(bagged[metric].iloc[best_idx] + pad * 0.25,
            bars[best_idx].get_y() + bars[best_idx].get_height() / 2 + 0.32,
    ...
    ax.set_title(f"v7: hierarchical with literature-backed features "
...
def main():
    parser = argparse.ArgumentParser(
        description="v7: hierarchical cascade with 3 new literature-backed feature blocks."
    )
    parser.add_argument("--data_dir", required=True)
    parser.add_argument(
        "--stage1",
        default="xgb:contrast+abcd_v2+boundary:100",
        help="Stage 1 (vasc vs rest) spec. Default = v3 winner.",
    )
    parser.add_argument(
        "--stage2",
        default="xgb:all+boundary+melnv+lbp_multi+gabor+subregion:120",
        help="Stage 2 (mel vs nv) spec. Default uses all 3 new feature blocks + melnv module + boundary.",
    )
    parser.add_argument(
        "--flat",
        default="xgb:all+contrast+abcd_v2+boundary+melnv+lbp_multi+gabor+subregion:150",
        help="Flat 3-class baseline. Default uses all features (incl. new blocks) for fair comparison.",
    )
    ...
    parser.add_argument("--output_csv", default="outputs/metrics/ml_hierarchical_v7.csv")
    parser.add_argument("--output_png", default="outputs/figures/ml_hierarchical_v7.png")
