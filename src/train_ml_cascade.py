...
try:
    ...
    raise ImportError(
        "xgboost is required for cascade training. "
...
def train_cascade(
    ...
    if labels is None:
        raise FileNotFoundError("label.csv is required for cascade training.")

    labels = labels.copy().reset_index(drop=True)
    labels["image_id"] = labels["image_id"].astype(str)
    y = labels["dx"].to_numpy()

    if stage2_variant not in STAGE2_VARIANTS:
        raise ValueError(f"Unknown stage2_variant: {stage2_variant}")
    ...
    stage2_model = _build_pipeline(
        STAGE2_VARIANTS[stage2_variant],
        n_classes=2,
        k_features=stage2_k_features,
    ...
    bundle = {
        ...
        "stage2": {
            ...
            "k_features": stage2_k_features,
            "xgb_kwargs": STAGE2_VARIANTS[stage2_variant],
    ...
    model_path = Path(model_path) if model_path else (
        DEFAULT_MODEL_DIR / f"xgb_cascade_{stage2_variant}_k{stage2_k_features}_soft.joblib"
    )
    ensure_dir(model_path.parent)
    joblib.dump(bundle, model_path)
    joblib.dump(bundle, DEFAULT_MODEL_DIR / "ml_cascade_candidate.joblib")
    print(f"Saved cascade candidate to {model_path}")
    print(f"Also updated {DEFAULT_MODEL_DIR / 'ml_cascade_candidate.joblib'}")
    return bundle


def main():
    parser = argparse.ArgumentParser(description="Train full-data XGBoost cascade candidate.")
    parser.add_argument("--data_dir", required=True)
    parser.add_argument("--mask_mode", default="raw", choices=["raw", "clean"])
    parser.add_argument("--stage2_variant", default="deeper", choices=sorted(STAGE2_VARIANTS))
    parser.add_argument("--stage2_k_features", default="120")
    ...
    train_cascade(
        ...
        stage2_k_features=args.stage2_k_features,
